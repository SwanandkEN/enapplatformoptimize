...
def run(self):
    ...
    # convert module
    module_trt = enaptrt(module, inputs_conversion, max_workspace_size=1 << 20,  **self.enaptrt_kwargs)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--output', '-o', help='Test output file path', type=str, default='enaptrt_test.md')
    ...
    parser.add_argument('--use_onnx', help='Whether to test using ONNX or enaptrt tracing', action='store_true')
    ...
    for test in MODULE_TESTS:
        ...
        try:
            if args.use_onnx:
                test.enaptrt_kwargs.update({'use_onnx': True})
                
            max_error, fps, fps_trt, ms, ms_trt = run(test)

            # write entry
            line = '| %s | %s | %s | %s | %.2E | %.3g | %.3g | %.3g | %.3g |' % (name, test.dtype.__repr__().split('.')[-1], str(test.input_shapes), str(test.enaptrt_kwargs), max_error, fps, fps_trt, ms, ms_trt)
            ...
            line = '| %s | %s | %s | %s | N/A | N/A | N/A | N/A | N/A |' % (name, test.dtype.__repr__().split('.')[-1], str(test.input_shapes), str(test.enaptrt_kwargs))
