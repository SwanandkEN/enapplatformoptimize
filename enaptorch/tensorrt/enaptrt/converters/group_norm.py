...
from enaptrt.enaptrt import *                                 
from enaptrt.module_test import add_module_test

def has_group_norm_plugin():
    try:
        from enaptrt.plugins import GroupNormPlugin
        return True
    except:
        return False
...
def get_group_norm_plugin(num_groups, weight, bias, eps):
    ...
    creator = [c for c in registry.plugin_creator_list if c.name == PLUGIN_NAME and c.plugin_namespace == 'enaptrt'][0]
    enaptrt_plugin = GroupNormPlugin(num_groups=num_groups, weight=weight, bias=bias, eps=eps)
    return creator.deserialize_plugin(PLUGIN_NAME, enaptrt_plugin.serializeToString())
